...
// Simulation for the PNG-based Ants display, with its own framebuffer.
...
namespace ants_v2_sim {

static const char *const TAG = "ants_v2_sim";

namespace Hardware {
// Logical landscape canvas, identical to the browser prototype.
constexpr int16_t kScreenWidth = 480;
constexpr int16_t kScreenHeight = 320;

// Native panel memory geometry. ESPHome's JC3248W535 model performs 90-degree
// drawing rotation in software; bulk draw_pixels_at() bypasses that software
// rotation, so our own framebuffer is stored pre-rotated in physical order.
constexpr int16_t kPhysicalWidth = 320;
constexpr int16_t kPhysicalHeight = 480;
}  // namespace Hardware

namespace AntSprite {
constexpr uint8_t kBodyWidth = 14;
constexpr uint8_t kBodyHeight = 19;
constexpr uint8_t kPadding = 2;
constexpr uint8_t kSpriteWidth = kBodyWidth + kPadding * 2;
constexpr uint8_t kSpriteHeight = kBodyHeight + kPadding * 2;
constexpr uint8_t kFrameCount = 2;
static constexpr uint16_t kRows[kFrameCount][kBodyHeight] = {
  { 0x0408, 0x0718, 0x03F0, 0x01E0, 0x31E0, 0x1DE3, 0x0FE7, 0x03FC, 0x07FF, 0x0FE3, 0x1FF0, 0x36F8, 0x0DEC, 0x1BF6, 0x33F3, 0x23F3, 0x03F0, 0x01F0, 0x01E0 },
  { 0x0618, 0x0618, 0x03F0, 0x01E0, 0x01E3, 0x31EE, 0x39FC, 0x0FF0, 0x3FF8, 0x31FC, 0x03FF, 0x07DB, 0x0DEC, 0x1BF6, 0x3BF3, 0x33F1, 0x03F0, 0x01F0, 0x01E0 }
};
inline bool body(uint8_t frame, int16_t x, int16_t y) {
  if (frame >= kFrameCount || x < 0 || x >= kBodyWidth || y < 0 || y >= kBodyHeight) return false;
  return (kRows[frame][y] & (1u << (kBodyWidth - 1 - x))) != 0;
}
inline uint8_t sample(uint8_t frame, int16_t x, int16_t y) {
  const int16_t bx = x - kPadding;
  const int16_t by = y - kPadding;
  if (body(frame, bx, by)) return 2;
  const int16_t sx = x - (kPadding + 1);
  const int16_t sy = y - (kPadding + 1);
  if (body(frame, sx, sy)) return 1;
  return 0;
}
}  // namespace AntSprite

namespace DigitLayouts {
struct Point { float x; float y; bool horizontal; };
struct Definition { const Point* points; uint8_t count; };
constexpr uint8_t kMaximumAntsPerDigit = 17;
static constexpr Point d0[] = {
  {0.18000000f, 0.00000000f, true},
  {0.50000000f, 0.00000000f, true},
  {0.82000000f, 0.00000000f, true},
  {0.12000000f, 0.16666667f, false},
  {0.12000000f, 0.33333333f, false},
  {0.12000000f, 0.50000000f, false},
  {0.12000000f, 0.66666667f, false},
  {0.12000000f, 0.83333333f, false},
  {0.88000000f, 0.16666667f, false},
  {0.88000000f, 0.33333333f, false},
  {0.88000000f, 0.50000000f, false},
  {0.88000000f, 0.66666667f, false},
  {0.88000000f, 0.83333333f, false},
  {0.18000000f, 1.00000000f, true},
  {0.50000000f, 1.00000000f, true},
  {0.82000000f, 1.00000000f, true},
};
static constexpr Point d1[] = {
  {0.58000000f, 0.00000000f, false},
  {0.58000000f, 0.16666667f, false},
  {0.58000000f, 0.33333333f, false},
  {0.58000000f, 0.50000000f, false},
  {0.58000000f, 0.66666667f, false},
  {0.58000000f, 0.83333333f, false},
  {0.58000000f, 1.00000000f, false},
};
static constexpr Point d2[] = {
  {0.18000000f, 0.00000000f, true},
  {0.50000000f, 0.00000000f, true},
  {0.82000000f, 0.00000000f, true},
  {0.88000000f, 0.16666667f, false},
  {0.88000000f, 0.33333333f, false},
  {0.18000000f, 0.50000000f, true},
  {0.50000000f, 0.50000000f, true},
  {0.82000000f, 0.50000000f, true},
  {0.12000000f, 0.66666667f, false},
  {0.12000000f, 0.83333333f, false},
  {0.18000000f, 1.00000000f, true},
  {0.50000000f, 1.00000000f, true},
...
static constexpr Point d4[] = {
  {0.12000000f, 0.00000000f, false},
  {0.12000000f, 0.16666667f, false},
  {0.12000000f, 0.33333333f, false},
  {0.18000000f, 0.50000000f, true},
  {0.50000000f, 0.50000000f, true},
  {0.82000000f, 0.50000000f, true},
  {0.88000000f, 0.00000000f, false},
  {0.88000000f, 0.16666667f, false},
  {0.88000000f, 0.33333333f, false},
  {0.88000000f, 0.66666667f, false},
  {0.88000000f, 0.83333333f, false},
  {0.88000000f, 1.00000000f, false},
};
static constexpr Point d5[] = {
  {0.18000000f, 0.00000000f, true},
  {0.50000000f, 0.00000000f, true},
  {0.82000000f, 0.00000000f, true},
  {0.12000000f, 0.16666667f, false},
...
static constexpr Definition kDigits[10] = {
  {d0, 16},
  {d1, 7},
  {d2, 13},
  {d3, 13},
...
constexpr float kDigitY = 94.0f;
constexpr float kDigitWidth = 60.0f;
constexpr float kDigitHeight = 132.0f;
...
constexpr uint8_t kTargetFps = 24;
...
inline uint16_t rgb565(const Rgb& color) {
  // ESPHome's mipi_spi JC3248W535 buffer defaults to big-endian RGB565.
  // ESP32 is little-endian, so store each 16-bit pixel byte-swapped; the bytes
  // in RAM are then [MSB, LSB], exactly what the display transport expects.
  const uint16_t native = static_cast<uint16_t>(((color.r & 0xF8u) << 8) |
                                                ((color.g & 0xFCu) << 3) |
                                                (color.b >> 3));
...
inline Rgb unpack565(uint16_t color) {
  // Convert our wire-order framebuffer value back to native RGB565 before
  // extracting channels for alpha blending.
  color = swap16(color);
...
inline size_t framebufferIndex(int16_t x, int16_t y) {
  // Same 90-degree software rotation ESPHome applies in MipiSpiBuffer::draw_pixel_at():
  // physical_x = native_width - logical_y - 1; physical_y = logical_x.
  const int16_t physicalX = Hardware::kPhysicalWidth - y - 1;
  const int16_t physicalY = x;
  return static_cast<size_t>(physicalY) * Hardware::kPhysicalWidth +
...
inline void clearFrame() {
  std::fill(gFramebuffer,
            gFramebuffer + static_cast<size_t>(Hardware::kPhysicalWidth) * Hardware::kPhysicalHeight,
...
inline bool begin() {
  ...
  const size_t bytes = static_cast<size_t>(Hardware::kPhysicalWidth) * Hardware::kPhysicalHeight * sizeof(uint16_t);
  gFramebuffer = static_cast<uint16_t*>(heap_caps_malloc(bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (gFramebuffer == nullptr) {
    gFramebuffer = static_cast<uint16_t*>(heap_caps_malloc(bytes, MALLOC_CAP_8BIT));
  }
  if (gFramebuffer == nullptr) {
    ESP_LOGE(TAG, "Unable to allocate %u-byte Ant Clock framebuffer", static_cast<unsigned>(bytes));
    return false;
  }
  ...
  initializeAnts();
  applyDisplayedTime(liveTimeNow(esphome::millis()), true);
  clearFrame();
  ...
  ESP_LOGI(TAG,
           "Renderer ready: logical 480x320 -> physical 320x480, big-endian RGB565, 24 FPS target, framebuffer=%u bytes",
...
inline void render(esphome::display::Display &it) {
  ...
  // draw_pixels_at() is a raw/native bulk transfer for mipi_spi; it does not
  // apply ESPHome's software display rotation. gFramebuffer is therefore already
  // stored in native 320x480 order. The JC3248W535 model expects big-endian RGB565.
  it.draw_pixels_at(0, 0,
                    Hardware::kPhysicalWidth, Hardware::kPhysicalHeight,
...
inline void touch_start(int16_t x, int16_t y) {
  if (!begin()) return;
  const uint32_t nowMs = esphome::millis();
  gPointer.x = clampFloat(static_cast<float>(x), 0.0f, Hardware::kScreenWidth - 1.0f);
  gPointer.y = clampFloat(static_cast<float>(y), 0.0f, Hardware::kScreenHeight - 1.0f);
...
inline void touch_move(int16_t x, int16_t y) {
  if (!gInitialized || gTouchConsumed) return;
  gPointer.x = clampFloat(static_cast<float>(x), 0.0f, Hardware::kScreenWidth - 1.0f);
  gPointer.y = clampFloat(static_cast<float>(y), 0.0f, Hardware::kScreenHeight - 1.0f);
  gPointer.inside = true;
  gPointer.down = true;
...
inline void set_live_mode() {
  if (begin()) setClockMode(ClockMode::Live, esphome::millis());
}

inline void next_minute() {
  if (begin()) advanceOneMinute(esphome::millis());
}

inline void toggle_demo() {
  if (!begin()) return;
  const uint32_t nowMs = esphome::millis();
  setClockMode(gClockMode == ClockMode::Demo ? ClockMode::Manual : ClockMode::Demo, nowMs);
}
